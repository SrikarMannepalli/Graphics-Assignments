...
Altitude::Altitude(float x, float y, float z) {
	...
	this->objectAlt = create3DObject(GL_TRIANGLES, n*3, vertex_buffer_data, COLOR_BACKGROUND, GL_FILL);
	this->objectAltMarker = create3DObject(GL_TRIANGLES, 2*3, vertex_buffer_data1, COLOR_DARKRED, GL_FILL);
}

void Altitude::draw(glm::mat4 VP2D) {
// 	if(camera_state == 0) this->rotation = 90.0f;
// 	else this->rotation = 0.0f;
	this->rotation = -(180.0*plane1.position.y/50);
	...
	draw3DObject(this->objectAlt);
	draw3DObject(this->objectAltMarker);
}
